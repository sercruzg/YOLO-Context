...
void forward_backward_select_network_gpu(network net, network contNet, network joinNet, float *x, float *y, network selectNet)
{
    network_state state;
    state.index = 0;
    state.objNet = net;
    state.contNet = contNet;
    state.joinNet = joinNet;
    state.net = selectNet;
    int y_size = get_network_output_size(selectNet)*selectNet.batch;
    if(selectNet.layers[joinNet.n-1].truths) y_size = selectNet.layers[selectNet.n-1].truths*selectNet.batch;
    //fprintf(stderr, "Truths %d\n",y_size);
    if(!*selectNet.truth_gpu){
        *selectNet.truth_gpu = cuda_make_array(y, y_size);
    }else{
        cuda_push_array(*selectNet.truth_gpu, y, y_size);
    }
    state.delta = 0;
    state.truth = *selectNet.truth_gpu;
    state.train = 1;
#ifdef CUDNN_HALF
	int i;
	for (i = 0; i < net.n; ++i) {
...
#endif
    //fprintf(stderr, "Join Forward\n");
    forward_network_gpu(selectNet, state);
    //fprintf(stderr, "Join Backward\n");
	cudaStreamSynchronize(get_cuda_stream());
    backward_network_gpu(selectNet, state);
}

void forward_backward_context_part_network_gpu(network net, network joinNet, float *x, float *y)
{
    network_state state;
    state.index = 0;
    state.net = joinNet;
    state.objNet = net;
    int x_size = get_network_input_size(joinNet)*joinNet.batch;
    ...
    //fprintf(stderr, "Join before\n");
    if(!*joinNet.truth_gpu){
        *joinNet.input_gpu = cuda_make_array(x, x_size);
        *joinNet.truth_gpu = cuda_make_array(y, y_size);
    }else{
        cuda_push_array(*joinNet.input_gpu, x, x_size);
        cuda_push_array(*joinNet.truth_gpu, y, y_size);
    }
    state.input = *joinNet.input_gpu;
    state.delta = 0;
...
void forward_backward_attention_network_gpu(network net, network contNet, float *x, float *y)
{
    //fprintf(stderr, "before Attention\n");
    network_state state, contState;
    state.index = 0;
    contState.index = 0;
    state.net = net;
    contState.net = contNet;
    state.contNet = contNet;
    int x_size = get_network_input_size(net)*net.batch;
    int y_size = get_network_output_size(net)*net.batch;
    if(net.layers[net.n-1].truths) y_size = net.layers[net.n-1].truths*net.batch;
    //fprintf(stderr, "before data\n");
    if(!*net.input_gpu){
        *net.input_gpu = cuda_make_array(x, x_size);
    ...
    if(!*contNet.input_gpu){
        *contNet.input_gpu = cuda_make_array(x, x_size);
    }else{
        cuda_push_array(*contNet.input_gpu, x, x_size);
    }
    //fprintf(stderr, "after data\n");
    state.input = *net.input_gpu;
    contState.input = *contNet.input_gpu;
    state.delta = 0;
    contState.delta = 0;
...
#endif
    forward_network_gpu(contNet, contState);
    cudaStreamSynchronize(get_cuda_stream());
    forward_network_gpu(net, state);
	cudaStreamSynchronize(get_cuda_stream());
    backward_network_gpu(net, state);
    cudaStreamSynchronize(get_cuda_stream());
    backward_network_gpu(contNet, contState);
    cudaStreamSynchronize(get_cuda_stream());
}
...
void forward_backward_context_network_gpu(network net, network contNet, network joinNet, float *x, float *y)
{
    network_state state;
    state.index = 0;
    state.net = joinNet;
...
void forward_backward_region_class_network_gpu(network net, network contNet , float *x, float *y, float *x_f)
{
    network_state state;
    state.index = 0;
    state.net = contNet;
    state.objNet = net;
    int y_size = get_network_output_size(contNet)*contNet.batch;
    int x_size = get_network_input_size(net)*net.batch;
    int x_f_size = get_network_input_size(contNet)*contNet.batch;
    if(contNet.layers[contNet.n-1].truths) y_size = contNet.layers[contNet.n-1].truths*contNet.batch;
    //fprintf(stderr, "Truths %d\n",y_size);
    if(!*contNet.truth_gpu){
        *net.input_gpu = cuda_make_array(x, x_size);
        *contNet.input_gpu = cuda_make_array(x_f, x_f_size);
        *contNet.truth_gpu = cuda_make_array(y, y_size);
    }else{
        cuda_push_array(*contNet.truth_gpu, y, y_size);
        cuda_push_array(*net.input_gpu, x, x_size);
        cuda_push_array(*contNet.input_gpu, x_f, x_f_size);
    }
    state.delta = 0;
    state.truth = *contNet.truth_gpu;
    state.train = 1;
#ifdef CUDNN_HALF
	int i;
	for (i = 0; i < contNet.n; ++i) {
		layer l = contNet.layers[i];
		cuda_convert_f32_to_f16(l.weights_gpu, l.c*l.n*l.size*l.size, l.weights_gpu16);
	}
#endif
    //fprintf(stderr, "Join Forward\n");
    forward_network_gpu(contNet, state);
    //fprintf(stderr, "Join Backward\n");
	cudaStreamSynchronize(get_cuda_stream());
    backward_network_gpu(contNet, state);
}

void forward_backward_deform_network_gpu(network net, network anchorNet, float *x, float *y)
{
    network_state state;
    state.index = 0;
    state.net = net;
    state.objNet = anchorNet;
    int x_size = get_network_input_size(net)*net.batch;
    int y_size = get_network_output_size(net)*net.batch;
    if(net.layers[net.n-1].truths) y_size = net.layers[net.n-1].truths*net.batch;
    if(!*net.input_gpu){
...
{
    *selectNet.seen += selectNet.batch;

    forward_only_network_gpu(net, x);
    forward_only_network_gpu(contNet, x);
    forward_only_network_gpu(joinNet, x);

    forward_backward_select_network_gpu(net, contNet, joinNet, x, y, selectNet);

    float error = get_network_cost(selectNet);
    if (((*selectNet.seen) / selectNet.batch) % selectNet.subdivisions == 0) {
        update_network_gpu(selectNet);
    }
    return error;
}
...
float train_network_context_datum_gpu_flip(network net, network contNet, network joinNet, float *x, float *y, float *x_f, float *y_f)
{
    ...
    *joinNet.seen += joinNet.batch;
    forward_backward_context_network_gpu(net, contNet, joinNet, x, y);
...
{
    forward_only_network_gpu(net, x);
    update_sum_output(net);
    update_all_index(net);
    //forward_backward_grid_network_gpu(net, x, y);
    *joinNet.seen += joinNet.batch;
    forward_backward_context_part_network_gpu(net, joinNet, x, y);
    float error = get_network_cost(joinNet);
    if (((*joinNet.seen) / joinNet.batch) % joinNet.subdivisions == 0) {
        update_network_gpu(joinNet);
    }
...
float train_network_attention_datum_gpu(network net, network contNet, float *x, float *y)
{
    //forward_only_network_gpu(contNet, x);
    *net.seen += net.batch;
    *contNet.seen += contNet.batch;
    //fprintf(stderr, "before net\n");
    forward_backward_attention_network_gpu(net, contNet, x, y);
    //fprintf(stderr, "after net\n");
    float error = get_network_cost(net);
    //fprintf(stderr, "before back\n");
    //backward_only_network_gpu(contNet);
    //fprintf(stderr, "after back\n");
    if (((*net.seen) / net.batch) % net.subdivisions == 0) {
        update_network_gpu(net);
        update_network_gpu(contNet);
    }
    return error;
}
...
float train_network_region_class_datum_gpu(network net, network contNet, float *x, float *y, float *x_f)
{
    ...
    *contNet.seen += contNet.batch;
    *net.seen += net.batch;
    //fprintf(stderr, "before second Forward\n");
    forward_backward_network_gpu(contNet, x_f, y);
    float error = get_network_cost(contNet);
    if (((*contNet.seen) / contNet.batch) % contNet.subdivisions == 0) {
        update_network_gpu(contNet);
    }
    return error;
}
...
{
    forward_only_network_gpu(anchorNet, x);
    *net.seen += net.batch;
    //fprintf(stderr, "Join Forward\n");
    forward_backward_deform_network_gpu(net, anchorNet, x, y);
    float error = get_network_cost(net);
    if (((*net.seen) / net.batch) % net.subdivisions == 0) {
...
float *network_predict_concat_gpu(network net, network contextNet, float *input)
{
    if (net.gpu_index != cuda_get_device())
		cuda_set_device(net.gpu_index);
    int size = get_network_input_size(net) * net.batch;
...
float *network_predict_flip_gpu(network net, network contextNet, network joinNet, float *input, float *input_f)
{
    if (net.gpu_index != cuda_get_device())
		cuda_set_device(net.gpu_index);
    int size = get_network_input_size(net) * net.batch;
    ...
    float *oriData_f = cuda_make_array(input_f, size);
    state.index = 0;
    state.net = contextNet;
    state.input = oriData_f;
    state.truth = 0;
    state.train = 0;
    state.delta = 0;
    forward_network_gpu(contextNet, state);
...
/*
float *network_predict_context_gpu(network net, network contextNet, network joinNet, network selectNet, float *input)
{
    if (net.gpu_index != cuda_get_device())
		cuda_set_device(net.gpu_index);
    int size = get_network_input_size(net) * net.batch;
    ...
    state.delta = 0;
    forward_network_gpu(joinNet, state);
    float *out = get_network_output_gpu(selectNet);
...
float *network_predict_context_gpu(network net, network contextNet, network joinNet, float *input)
{
    if (net.gpu_index != cuda_get_device())
		cuda_set_device(net.gpu_index);
    int size = get_network_input_size(net) * net.batch;
