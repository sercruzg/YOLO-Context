...
void grad_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, int dont_show, char *buff)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    char **names = get_labels(name_list);
    ...
    set_batch_network(&net, 1);
    layer l = net.layers[net.n - 1];
    convolutional_layer first_l = net.layers[0];
    srand(2222222);
    clock_t time;
    char label[4096];
    find_replace(buff, "images", "labels", label);
    //char buff[256] = "/public/scruzgome2/egoDailyDatabase/images/subject1/eating/eating1/frame136.jpg";
    //char label[256] = "/public/scruzgome2/egoDailyDatabase/labels/subject1/eating/eating1/frame136.txt";
    find_replace(label, ".jpg", ".txt", label);
    char *input = buff;
    
    int count = 0;
    printf("Boxes loaded \n");
    box_label *boxes = read_boxes(label, &count);
    printf("Boxes loaded \n");
    int j,k;
    float nms=.4;
    int counter = 1;
...
//         strtok(input, "\n");
        ...
        image sized = resize_image(orig, net.w, net.h);
        save_image_jpg(sized, "./test4");
        float *X = sized.data;
        
       int gtCount = 0;
       for(int k = 0; k < count; ++k){
           gtCount = k*5;
           net.gt[gtCount] = boxes[k].x;
           net.gt[gtCount + 1] = boxes[k].y;
           net.gt[gtCount + 2] = boxes[k].w;
           net.gt[gtCount + 3] = boxes[k].h;
           net.gt[gtCount + 4] = boxes[k].id;
       }
       net.gtMax = count;

        //struct load_args *ptr = calloc(1, sizeof(struct args));
        printf("Loading\n");
        //load_grad_im(args);
...
//         time=clock();
        ...
        printf("Calculating grad\n");

//         get_next_batch(d, 1, 0, X, y);
        *net.seen = net.batch;
        forward_backward_grad_network_gpu(net, X);
        
        //sized.data = *X;
        //image test = make_random_image(net.w, net.h, 3);
        image test = float_to_image(first_l.w,first_l.h,first_l.c,first_l.delta);
        //memcpy(test.data, X, d.X.cols*sizeof(float));
        save_image_jpg(test, "./test");
        image test2 = float_to_image(first_l.w,first_l.h,first_l.c,first_l.output);
//         image test2 = first_l.get_convolutional_image(first_l);
        save_image_jpg(test2, "./test2");
        image test3 = float_to_image(net.w,net.h,3,X);
        save_image_jpg(test3, "./test3");
        free(boxes);
